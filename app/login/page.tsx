...
export default function LoginPage() {
  ...
  async function onLogin(e: React.FormEvent) {
    ...
    // ✅ garante que a sessão está disponível (mobile-safe)
  ...
  return (
    <div style={{ padding: 24, maxWidth: 460 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <button onClick={() => { setMode("login"); setMsg(""); }} style={{ padding: "8px 10px", borderRadius: 10, fontWeight: 700, opacity: mode === "login" ? 1 : 0.6 }} type="button">
          Entrar
        </button>
        <button onClick={() => { setMode("reset"); setMsg(""); }} style={{ padding: "8px 10px", borderRadius: 10, fontWeight: 700, opacity: mode === "reset" ? 1 : 0.6 }} type="button">
          Esqueci minha senha
        </button>
      </div>
