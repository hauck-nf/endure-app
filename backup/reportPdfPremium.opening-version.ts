...
export async function buildEndurePremiumPdf(params: {
...

  {
    ...
    for (const [k, v] of rows) {
      ...
      page.drawText(v, { x: 210, y, size: 10.5, color: rgb(0.07, 0.09, 0.15) });
      y -= 15;
    }
...

  {
    ...
    for (const s of strengths.slice(0, 7)) {
      page.drawText(`• ${s}`, { x: 52, y: y1, size: 10, color: rgb(0.12, 0.16, 0.22) });
      y1 -= 15;
    }
    if (strengths.length === 0) {
      page.drawText("Nenhuma escala em faixa alta.", { x: 52, y: 192, size: 10, color: rgb(0.12, 0.16, 0.22) });
    }

    let y2 = 192;
    for (const s of potentials.slice(0, 7)) {
      page.drawText(`• ${s}`, { x: 322, y: y2, size: 10, color: rgb(0.12, 0.16, 0.22) });
      y2 -= 15;
    }
    if (potentials.length === 0) {
      page.drawText("Nenhuma escala em faixa baixa.", { x: 322, y: 192, size: 10, color: rgb(0.12, 0.16, 0.22) });
    }
  ...
  for (const f of factors.sort((a, b) => String(a.score_scale).localeCompare(String(b.score_scale)))) {
    ...
    for (const [k, v] of meta) {
      ...
      page.drawText(v, { x: mx, y: 655, size: 14, color: rgb(0.07, 0.09, 0.15) });
      mx += 120;
    }
